...
このリポジトリは、その仮説を確認するための**小規模な診断実験専用**のコード一式。
FT-Transformer のアーキテクチャはそのままに、optimizer backbone だけを
AdamW → SGD に差し替えて、CTRL の介入挙動 (kurtosis の推移・intervention 回数) が
変化するかを見る。

**Axis B 本実験 (`train_tabular_ft_transformer.py`) とは完全に別物であり、ここでの
結果は本実験の結果とは混同しない。** CTRL 自体
...
- `run_sgd_diag_tabular.sh` — 実行用シェルスクリプト。Axis C (`run_pilot_text.sh`)
  と同じく、自分自身のいるディレクトリを検出して `git pull` してから実行する。
  `smoke` (seed 0 のみ) と `full` (seed 0/1/2) の2モード。
- `gala_optimizer.py` / `cwgd_controller.py` — `train_tabular_sgd_diag.py` が
  import 時に必要とする依存モジュール (中身は本実験用と同一、未変更)。

## セットアップ (ラボサーバ、最初の1回だけ)

```bash
git clone <このリポジトリのURL> /data01/s_hihara/ctrl_v6_sgd_diag
cd /data01/s_hihara/ctrl_v6_sgd_diag
bash run_sgd_diag_tabular.sh smoke > sgd_diag_smoke_log.txt 2>&1
cat sgd_diag_smoke_log.txt
```

2回目以降は同じディレクトリで `bash run_sgd_diag_tabular.sh smoke|full` を実行
するだけでよい (起動時に自動で `git pull` する)。
